...
MIDI_HI = 155           # B11 -- as high as the SN76489 goes (period ~2); useful
                        #     for pitched noise.
MIDI_WRAP = 131         # B9 (index 86): notes from here up are shown with a '+'
                        #     and re-labelled 7 octaves down (B9 -> B-2+, A#10 ->
                        #     A#3+), so the class is preserved and the octave digit
                        #     stays single. WBASE = 7 octaves = the re-label shift.
NAMES = ["C-", "C#", "D-", "D#", "E-", "F-",
         "F#", "G-", "G#", "A-", "A#", "B-"]
...
def main():
    ...
    lines.append(".DEFINE NOTE_WRAP %d" % (MIDI_WRAP - MIDI_LO))  # first '+' note index
    lines.append(".DEFINE NOTE_WBASE %d" % (7 * 12))              # re-label shift (7 octaves)
