...
.RAMSECTION "vars" SLOT 3
  ...
  xport_seen     db           ; 0 until the transport is first used (gates STOP/PLAY glyph)
  help_banner    db           ; frames left on the boot "HOLD 2 TO VIEW HELP" banner
...
.ENDIF
  ...
  ; NB: no state_dirty here -- the STOP/PLAY glyph stays hidden until the
  ; transport is first used (xport_seen). The SONG title row instead shows the
  ; "HOLD 2 TO VIEW HELP" hint for ~3 s (help_banner), then repaints normally.
  ld a, 180
  ld (help_banner), a
...
main_loop:
  ...
  call draw_help_banner        ; boot "HOLD 2 TO VIEW HELP" hint (SONG title row, ~3 s)
...
tp_dirty:
  ld a, 1
  ld (state_dirty), a
  ld (xport_seen), a         ; transport used at least once -> glyph is live now
  ret
...
; draw_state lives in bank 1 (editor.asm "Help" section) to spare bank-0 space;
; the main loop reaches it via a plain call.
...
.INCLUDE "notes.inc"
.INCLUDE "buildid.inc"
.INCLUDE "help.inc"        ; HELP-screen text (generated from help.txt)
