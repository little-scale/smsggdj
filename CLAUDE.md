...
Releases are versioned — `str_version` in `src/main.asm` is the splash string (currently `V0.28`, dev), with a per-version `CHANGELOG.md`. Add user-facing bullets to the current version's section as you make changes, and keep `str_version` in sync on a release. To cut a release: date the CHANGELOG section, `make dist` (builds the version-stamped `build/smsggdj_<ver>.sms` / `.gg`), tag `v0.NN`, and `gh release create v0.NN` attaching those **versioned** ROMs (filenames carry the version; the bare `smsggdj.sms`/`.gg` are for local use only).
