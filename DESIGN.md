...
- **CONT — continuous play across LOAD** (PROJECT: `OFF / T1 / T2 / T3 / NO`): the transport keeps running through FILES and a LOAD, for DJ-style set transitions. **On LOAD in SONG mode every track re-enters the new song at its top (first populated cell at/below song row 0) but keeps its within-phrase step** (`chan_row` is preserved), so the incoming song is **beat-matched** — no phase reset. **In LIVE mode the non-carried tracks are instead silenced on the load** (`cont_restart_all`'s LIVE branch calls `live_track_stop` on each track but the carried one) — playback is performer-driven, so the other parts don't auto-play the new song; you re-arm them by queuing chains (`lq_arm` reactivates a silent track). The channel named by CONT has its currently-playing phrase stashed pre-swap and planted in the **top phrase (`NUM_PHRASES-1` = 51) / chain (`NUM_CHAINS-1` = 39)**, **borrowed as scratch**, as a bridge that sounds through the load, then merges back at the new song's top (in LIVE the bridge chain **loops in place** until new material is queued, via `atn_loop`). The **top instrument (`NUM_INSTR-1` = 15)** is borrowed the same way so the bridge keeps its **timbre**: on stash, the carried instrument (the first note's, else the channel's live `ix+1`) is snapshotted into `carry_instr` and every explicit instrument byte in the carried phrase is rewritten to slot 15; on plant, `carry_instr` is baked into instrument 15 and the carried channel's `ix+1` is set to 15 (so "inherit" steps use it too). Without this the bridge would inherit whatever the *new* song has at those instrument numbers. **All slots stay fully usable** — `USER_PHRASES`/`USER_CHAINS`/`USER_INSTR` equal the pool counts (52 / 40 / 16), so the editor caps nothing; the handoff just overwrites the top slot of each during a load (no collision check), so a song that both fills those top slots and is performed with CONT will have them clobbered. (Not carried: a table the instrument references still reads the new song's table at that index. And because a song may legitimately place chain 39, the `ix+11 == NUM_CHAINS-1` bridge-detection heuristic in `pu_song`/`song_bridge_cues` can false-positive on such a track — cosmetic only.) Queued LIVE cells and the wrap point (`eng_len`) / groove are rebased onto the new song. Samples are muted while FILES maps cart SRAM over the pool. **UI:** because the bridge chain is off-grid, `pu_song` suppresses the SONG grid playhead for a track whose `ix+11` is the reserved chain (a cell marker would falsely read as "playing" and block re-triggering it); instead a `>` is drawn beside that track's header name (`dl_so_hdr`) so the performer can see which track carries the groove. Implementation: `load_carry_pre` (+ `carry_bake_instr`) / `load_rebase` → `cont_restart_all` + `load_carry_post` (bank 1); `pu_song` also parked in bank 1. **Tempo glide (prototype):** since tempo *is* the groove (integer frames-per-row at the fixed frame rate), a CONT load **ramps** the tempo over the **PROJECT SLID field** (`glide_len` bars, default `GLIDE_BARS`=4, `OFF`=instant) rather than jumping — `load_carry_pre` snapshots the old groove's average frames-per-row (`groove_avg_calc`) before the swap, `cont_tempo_glide` sets up a Bresenham ramp to the new groove's average, and a flat **scratch groove** (selected via a `groove_sel == NUM_GROOVES` sentinel in `groove_base`) is rewritten each bar by `glide_bar` at the `engine_tick` wrap, handing off to the real groove at the end (`glide_abort` also clears it on stop). Skipped when slaved or when tempos match. MVP steps whole rungs (coarse on big jumps); a fractional-average scratch groove would smooth it.
