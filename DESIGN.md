...
- **CONT — continuous play across LOAD** (PROJECT, drawn as four glyphs, e.g. `----` / `1---` / `1-3N`): a **4-bit handover mask** — `b0`=T1 `b1`=T2 `b2`=T3 `b3`=N, `0`=OFF. The transport keeps running through FILES and a LOAD, for DJ-style set transitions, and **every set bit carries that track's playing phrase across the swap** as an independent bridge. **Up to two tracks carry at once** (`NCARRY`=2): the selector (`pco_valid`) only scrolls through reachable combos — OFF, any single, or any pair — and carried tracks are **packed** into `NCARRY` private buffers (`carry_buf` = `NCARRY×64`, `carry_instr` = `NCARRY×16`) via a channel→slot map `carry_slot[4]`, so any channel can be chosen; only the count is capped (a RAM budget: the 6912-byte song block + stack leave room for two, not four — see the note below). CONT is **not persisted** — it always boots OFF (a per-set performance choice; also avoids reinterpreting a pre-mask 0–4 config byte as a mask). **On LOAD in SONG mode every track re-enters the new song at its top (first populated cell at/below song row 0) but keeps its within-phrase step** (`chan_row` is preserved), so the incoming song is **beat-matched** — no phase reset. **In LIVE mode the non-carried tracks are instead silenced on the load** (`cont_restart_all`'s LIVE branch, `cra_live`, calls `live_track_stop` on every track whose `carry_slot[C]==$FF`) — playback is performer-driven, so the other parts don't auto-play the new song; you re-arm them by queuing chains (`lq_arm` reactivates a silent track). Each carried channel has its phrase stashed pre-swap (`carry_buf + slot*64`) and its instrument snapshotted (`carry_instr + slot*16` — the first note's, else the channel's live `ix+1`), and is put in **bridge mode**: `ix+11 == BRIDGE_MK` (`$FE`) and `ix+1 == NUM_INSTR`. Those are **sentinels, not pool slots** — `process_chan` reads the step from `carry_buf` (indexed via `carry_slot[cur_trig_ch]`), `instr_rec` returns the matching `carry_instr`, and `advance_track` routes `BRIDGE_MK` to `at_bridge` (LIVE: loop the buffer / take a queue; SONG: merge into the new song at `ix+7` = the row-0 target). So the bridge **touches no song data at all**: all 52/40/16 slots stay fully usable and editable during CONT, the load doesn't dirty the song, and the `BRIDGE_MK` detection never collides with a real chain. `live_track_stop` / `lq_arm` **clear `ix+11`** when they silence or re-arm a track, so a stale bridge sentinel can't trap a track that isn't re-carried on a later load. Queued LIVE cells and the wrap point (`eng_len`) / groove are rebased onto the new song. Samples are muted while FILES maps cart SRAM over the pool. **UI:** because the bridge is off-grid, `pu_song` suppresses the SONG grid playhead for a `BRIDGE_MK` track; `song_bridge_cues` draws a `>` to the left of a bridging track's header name and a `*` to the right of **every** CONT-masked track. Implementation: `load_carry_pre` (loop over the mask → `carry_snapshot_chan` + `carry_bake_instr`) / `load_rebase` → `cont_restart_all` + `load_carry_post` (bank 1); `at_bridge` / `pu_song` also parked in bank 1.
    - **Beat-quantized LOAD (CUED).** With CONT on and playing, LOAD **queues** the swap for the next phrase boundary — the lowest carried track's `chan_row` 15→0 edge (`cont_load_ref`, edge-detected in `cont_load_service`, called each frame from `editor_input`) — so the swap and the tempo glide land on the downbeat. The queue **survives leaving FILES**: `cont_load_fire` does its own `smp_abort` + SRAM map + pool restore (gated on `sram_live` — inside FILES it's already mapped), so it fires from any screen. Stopped, or CONT off → immediate load (`fpx_load` falls through to the old path). The FILES count line shows **`CUED nn IN xx`** (queued slot, phrase rows left) and, once the glide is running, **`MATCH IN nnn`** (rows of tempo glide left); both animate per-row (`mark_dirty_a` on row change). They also appear (compact — `CUE IN xx` / `MATCH nnn`) on the **SONG** title row via `song_cue_draw` (called in the draw phase after `editor_draw`, self-refreshing, placed before the transport glyph on both flavors), so the countdown is visible after you back out of FILES.
    - **Tempo glide.** Since tempo *is* the groove (integer frames-per-row at the fixed frame rate), a CONT load **ramps** the tempo over the **PROJECT SLID field** (`glide_len` bars, default `GLIDE_BARS`=4, `OFF`=instant) rather than jumping — `load_carry_pre` snapshots the old groove's average frames-per-row (`groove_avg_calc`) before the swap, `cont_tempo_glide` sets up a Bresenham ramp to the new groove's average, and a flat **scratch groove** (selected via a `groove_sel == NUM_GROOVES` sentinel in `groove_base`) is rewritten each bar by `glide_bar` at the `engine_tick` wrap, handing off to the real groove at the end (`glide_abort` also clears it on stop). Skipped when slaved or when tempos match. MVP steps whole rungs (coarse on big jumps); a fractional-average scratch groove would smooth it.
    - **RAM/decode notes.** The two private buffers fit the ~8 KB work RAM alongside the song block and the stack (four buffers overran the stack — hence `NCARRY`=2). The residual load-decode stall was addressed not by the designed **staggered load** (STAGGERED_LOAD.md, unbuilt/superseded) but by making `rle_unpack` register-resident *and* skipping the discarded post-decode checksum on the interactive path (`rle_nocheck`) — ~6→~1 frame. An in-flight bridge **is preserved across a back-to-back load**: `load_carry_pre` runs two passes — pass 1 keeps a still-bridging track that's still in the mask (its private buffer survives the swap, so its `carry_slot[C]` is left as-is), pass 2 packs fresh snapshots into the remaining free slots (`carry_free_slot`). So loading again while a track is mid-bridge no longer drops it to silence.
...
| **OPTIONS** | The machine/rig page, persisted in the config block (SAVEFORMAT.md): **VIDEO: AUTO/PAL/NTSC**, SRAM readout, **SYNC: OUT/PULSE/IN/OFF**, **COLR** (8 palettes 0-7, patchable via tools/palette.html), **CLONE: SLIM/DEEP** (§12), **FM** on/off, **RDLY/RSPD** (cursor key-repeat delay 1-60 / interval 1-30, in frames) |
...
Needs the SMS **FM Sound Unit** (ports `$F0`/`$F1`/`$F2`), enabled by **OPTIONS → FM** (persisted; default OFF). Pitch comes from a region F-number/block table (`maketables.py`, like the PSG note table). The FM table is voiced **`FM_OCT_SHIFT` = 2 octaves below** the PSG note of the same index (so note index 0, shown `A-2`, sounds **A0 ≈ 27.5 Hz**), letting the shared note field reach FM's whole range — and *below* the PSG floor — without extending the note table. With a patch **MUL = 0** (×0.5 operator multiplier) the carrier drops another octave, so the **lowest FM carrier is ≈ 13.75 Hz** (sub-audio — useful as an LFO-ish rumble). A track hosting an FM instrument plays on the FM channel of the same index and **silences its PSG voice**; the FM note uses the chip's own envelope (no per-tick engine work beyond the HLD key-off). `$F2` is a PSG/FM mux on the external unit (and Emulicious) but sums on the built-in FM hardware (and SMSPlus). **Hardware-verified on a PAL SMS1 + FM Sound Unit** — melodic FM, FMDRM drums, custom presets, and the `X`/`Y`/table commands all confirmed on real silicon.
...
**FM drums (rhythm mode).** A second FM instrument type, **FMDRUM** (type 5), drives the YM2413's rhythm mode (`$0E` bit 5): the chip's 5 fixed percussion voices — **BD, SD, TT** (tom), **TC** (cymbal), **HH** (hi-hat). It's a *kit*: one instrument, the **note picks the drum** (note % 12 cycles BD/SD/TT/TC/HH every 5 semitones — C/C♯/D/D♯/E in any octave). Rhythm mode uses FM channels 6–8, which the tracker never allocates for melodic FM (only 0–3), so it's enabled permanently alongside melodic FM at **zero cost** to the 4 tracks. Trigger pulses the drum's `$0E` key-on bit (off→on edge = re-attack); the drum's level lands in the shared `$36/$37/$38` register via a RAM shadow (the two-drum-per-register packing means we can't read-modify-write the chip). **VOL** sets the level, **HLD** the length (`F` = ring per the chip's percussion envelope, `1`–`E` = key-off after nibble×2 ticks). No transpose, no tables, no per-note program. A **DRUM** field (instrument byte +4) selects the mode: `ALL` (default — the kit behaviour above, fixed pitches from `fm_rhythm_tab` at FM init) or a **single drum** (`BD`/`SD`/`TOM`/`TCY`/`HH`). With a single drum picked, the **note's F-number/block drives that drum's rhythm-channel carrier** (`$16/$26` ch6, `$17/$27` ch7, `$18/$28` ch8) via `fm_drum_pitch`, so the note *tunes* the drum's operators (the little-scale YM2413 drum-tuning trick). The channels are shared — **BD→ch6, SD/HH→ch7, TOM/TCY→ch8** — so two of a shared pair can't hold different pitches simultaneously. Note the **HH (and cymbal) are the noise-based rhythm voices**, so they respond far *less* to the carrier pitch than the tonal BD/SD/TOM — that's inherent to the YM2413's rhythm operator design, not a bug.
